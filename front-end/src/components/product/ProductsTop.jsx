...
const DisplayContainer = styled.div`
    ...
    border-left: .5px solid ${props => props.theme.palette.divider};
...
const Percentage = styled.span`
    ...
    font-size: 14px;
    font-weight: bold;
    background-color: ${props => alpha(props.theme.palette.success.light, .8)};
...
const tempProduct = (
    <Display>
        ...
        <InfoWrapper>
            ...
            <ProductShop><Skeleton variant="text" width="30%" /></ProductShop>
            <Price><Skeleton variant="text" width="15%" /></Price>
            <DiscountContainer>
...
const ProductsTop = ({ data, isError, isLoading, isFetching, isSuccess, isUninitialized = false, size = 5, scrollPosition }) => {
    const loading = (isLoading || isFetching || isError || isUninitialized);
    const [selected, setSelected] = useState(data?.ids[0] ?? -1);

    useEffect(() => { setSelected(data?.ids[0] ?? -1); }, [data])
