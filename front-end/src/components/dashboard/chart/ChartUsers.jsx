...
const renderActiveShape = (props) => {
    ...
    return (
        <g>
            <text x={cx} y={cy} dy={-15} textAnchor="middle" fill={fill} fontWeight={'bold'}>
                {payload.name}
            </text>
            <text x={cx} y={cy} dy={5} textAnchor="middle" fill={'black'} fontWeight={'bold'}>
                {`${value.toLocaleString()} đ`}
            </text>
            <text x={cx} y={cy} dy={25} textAnchor="middle" fill={'black'} fontWeight={'bold'}>
...
const CustomPieTooltip = ({ active, payload }) => {
    if (active && payload && payload.length) {
        return (
            <TooltipContainer>
                <NameText>
                    <Avatar sx={{ marginRight: 1 }}>{payload[0]?.name?.charAt(0) ?? ''}</Avatar>
                    {payload[0].name}
                </NameText>
                <p style={{ margin: 0 }}>{getIntroOfPage(payload[0]?.dataKey, payload[0]?.value)}</p>
            </TooltipContainer>
        );
    }
            ...
            <Grid size="grow">
                <Paper elevation={3} sx={{ pb: 3, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                    ...
                    <ResponsiveContainer width="95%" height={350}>
                        <BarChart data={usersData} margin={{ top: 0, right: 0, left: 15, bottom: 15 }}>
