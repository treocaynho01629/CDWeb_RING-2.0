...
public class CategoryController {
	...
	public ResponseEntity<?> getPreviewCategories(){
		List<CategoryDTO> categories =  cateService.getPreviewCategories();
		return new ResponseEntity< >(categories, HttpStatus.OK);
	}

	//Get categories
	@GetMapping
	public ResponseEntity<?> getCategories(@RequestParam(value = "pSize", defaultValue = "10") Integer pageSize,
										@RequestParam(value = "pageNo", defaultValue = "0") Integer pageNo) {
		return new ResponseEntity<>(cateService.getCategories(pageNo, pageSize), HttpStatus.OK);
	}

	//Get category by {id}
	@GetMapping("/{id}")
	public ResponseEntity<?> getCategoryById(@PathVariable("id") Integer id) {
		return new ResponseEntity<>(cateService.getCategoryById(id), HttpStatus.OK);
	}
